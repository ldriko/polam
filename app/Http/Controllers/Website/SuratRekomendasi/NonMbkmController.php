...
{
    function index() {
        $guide = Guide::where('type', Submission::TYPES[11])->first();
        $data = Submission::where('user_id', Auth::id())->where('type', Submission::TYPES[11])->with('user')->orderBy('created_at', 'desc')->paginate(10);
        return view('website.surat-rekomendasi.non-mbkm.index', compact('data', 'guide'));
    }
    ...
    function store(Request $request) {
        $request->validate([
            ...
            'program_organizer' => ['required', 'string'], 
            ...
            'program_name.string' => 'Format nama program tidak valid',
            'program_organizer.required' => 'Penyelenggara program wajib diisi',
            'program_organizer.string' => 'Format penyelenggara program tidak valid',
        ...
        $create = Submission::create([
            'user_id' => Auth::id(),
            'type' => Submission::TYPES[11],
            'data' => json_encode($request->except('_token')),
        ]);

        if ($create) {
            return redirect()->route('surat-rekomendasi.non-mbkm.index')->with([
                'status' => 'success',
