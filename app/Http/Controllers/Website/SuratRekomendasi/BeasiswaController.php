...
{
    function index() {
        $data = Submission::where('user_id', Auth::id())->where('type', Submission::TYPES[9])->with('user')->orderBy('created_at', 'desc')->get();
        return view('website.surat-rekomendasi.beasiswa.index', compact('data'));
    }
